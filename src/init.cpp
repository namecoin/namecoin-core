...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    gArgs.AddArg("-prunenamehistory=<n>",
                 strprintf("If -namehistory is enabled, trim individual name_history entries once they are more than (expirationDepth + <n>) blocks deep, applied uniformly to entries of live, expired, and renewed names. The trim is driven by a dedicated history-tail index that records every archived entry by its own write height, so entries on actively renewed names are dropped on schedule and do not accumulate. The current name_show row is never touched by the trim. Pre-existing name_history rows written before this option was first enabled are not indexed and will not be trimmed until the next name update on that name; reindex if you want immediate effect on legacy data. 0 = disabled. Node-local, irreversible across reorgs (default: %u)", 0),
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
